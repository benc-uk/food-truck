...
- In addition my [own project starter template](https://github.com/benc-uk/project-starter) and [Go REST API for bookstore](https://github.com/benc-uk/go-rest-books) were used to bring a lot of reuseable assets and source.
...
- SQLite DB was used rather than a CSV file, this was sourced from here https://san-francisco.datasettes.com/food-trucks
...
- Standard REST API pattern was used
- Only queries GETs are used with a single `/trucks` endpoint
- See [api/spec.yaml](./api/spec.yaml) for the OpenAPI description of the API, this is auto generated by running `make generate`
- See the [API docs for further details](./api/)

### Limitations, Known Issues & Backlog

- The query for finding nearby trucks is *extremely* suboptimal and a borderline hack. Switching to a database service with spatial support like Cosmos, PostgreSQL or Azure SQL should a highest priority
...
| DATABASE_PATH | Some very important setting. **_Required_** | _None_ |
| FRONTEND_DIR | Some less important setting | "foo" |
